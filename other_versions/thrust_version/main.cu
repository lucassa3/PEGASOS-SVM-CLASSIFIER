...
int main(int argc, char *argv[]) {
    ...
    vector<vector<double>> h_data = read_data("../datasets/iris.data.txt", 500000,  0);
    ...
    thrust::device_vector<double> x_train(data.begin(), data.begin() + (data.size()/5) - ((data.size()/5)%f_size));
    thrust::device_vector<double> x_test(data.begin() + (data.size()/5) - ((data.size()/5)%f_size), data.end());

    thrust::device_vector<int> y_train(labels.begin(), labels.begin() + labels.size()/5);
    thrust::device_vector<int> y_test(labels.begin() + labels.size()/5, labels.end());
    ...
    for (unsigned int i = 0; i < 1; i++) {
        ...
        SVMClassifier* svm_clf = new SVMClassifier(1, 40000, 42, f_size, 20);
