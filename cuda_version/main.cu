...
void set_envs(string & data_path, double *c, unsigned int *samples_limit, unsigned int *epochs, unsigned int *batch_size, unsigned int *train_size, unsigned int *num_iterations) {
    ...
    char *data_path_env, *samples_limit_env, *epochs_env, *batch_size_env, *train_size_env, *num_iterations_env;
...
int main(int argc, char *argv[]) {
    ...
    for (unsigned int i = 0; i < num_iterations; i++) {
        ...
        SVMClassifier* svm_clf = new SVMClassifier(c, epochs, time(NULL), f_size, batch_size);
